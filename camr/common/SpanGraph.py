...
class SpanNode(object):
    ...
    def __str__(self):
        return 'Span node:(%s,%s) Children: %s SWAPPED:%s' % (
        self.start, self.end, self.children, self.SWAPPED)
...
class SpanGraph(object):
    ...
    def init_ref_graph(amr, alignment, sent=None):
        ...
        if amr.roots[0] in span_alignment:
            ...
            if debuglevel > 1:
                print("GraphID:%s WARNING:root %s not aligned!" % (
                SpanGraph.graphID, amr.node_to_concepts[amr.roots[0]]), file=log)
    ...
    def fix_root(self):
        ...
        for node_id in self.nodes:
            if self.nodes[
                node_id].parents == [] and node_id != 0 and not self.isContained(
                    node_id):  # multi-roots
        ...
        for h in varables:
            ...
            if not h in span_alignment:  # unaligned concept
                ...
                if not hspan.start in spgraph.nodes:
                    h_node = SpanNode.from_span(hspan)
                    spgraph.add_node(h_node)
                    if hconcept == 'and': and_nodes.add(
                        h_node.start)  # aligned 'and' nodes
                h_index = hspan.start
    ...
    def init_dep_graph(instance, sent=None):
        ...
        for tok in instance.tokens:
            if tok['id'] == 0:  # root
                ...
                if 'head' not in instance.tokens[
                    gov_id] and gov_id not in dpg.multi_roots:
    ...
    def add_coref_info(self, instance):
        if instance.coreference:
            for cpair in instance.coreference:
                ...
                if (not self.sent[src_pos]['pos'].startswith('PRP') and not
                self.sent[sink_pos]['pos'].startswith('PRP') and src_pos < sink_pos) or (
                self.sent[sink_pos]['pos'].startswith('PRP')):
                    ...
                    if sink_head != src_pos: src_node.incoming_traces.add(
                        (sink_rel, sink_head))
                elif (not self.sent[src_pos]['pos'].startswith('PRP') and not
                self.sent[sink_pos]['pos'].startswith('PRP') and sink_pos < src_pos) or (
                self.sent[src_pos]['pos'].startswith('PRP')):
                    while self.sent[src_head]['pos'] in constants.FUNCTION_TAG:
                        src_rel = self.sent[src_head]['rel']
                        src_head = self.sent[src_head]['head']
                    if src_head != sink_pos: sink_node.incoming_traces.add(
                        (src_rel, src_head))
                else:
                    pass
    ...
    def clear_up(self, parent_to_add, cidx):
        ...
        '''
        deleted_nodes = set([])
        if isinstance(parent_to_add, int) and isinstance(cidx, int) and self.sent[cidx][
            'ne'] != 'O':
            ...
            for c in self.nodes[parent_to_add].children:
                if isinstance(c, int):
                    pcnode = self.nodes[c]
                    pctok_set = set(
                        self.sent[j]['form'] for j in range(pcnode.start, pcnode.end))
                    if self.sent[c]['ne'] == self.sent[cidx]['ne'] and self.sent[c][
                        'ne'] and len(ctok_set & pctok_set) > 0:
    ...
    def merge_node(self, idx1, idx2):
        ...
        self.nodes[idx1].end = self.nodes[idx2].end if self.nodes[idx1].end < self.nodes[
            idx2].end else self.nodes[idx1].end
        ...
        self.nodes[idx1].incoming_traces = self.nodes[idx1].incoming_traces | self.nodes[
            idx2].incoming_traces
        self.remove_node(idx2)

    def get_multi_roots(self):
        multi_roots = []
        for n in list(self.nodes.keys()):
            if self.nodes[n].parents == [] and self.nodes[
                n].children != []:  # root TODO: Detect root with circle
    ...
    def tuples(self):
        ...
        for n in sorted(self.nodes.keys()):
            if (self.nodes[n].parents == [] or n not in node_set) and self.nodes[
                n].children != []:  # root
    ...
    def locInTree(self, idx):
        ...
        while idx not in candidates:
            candidates = sorted(list(set(
                [self.nodes[l].parents[0] for l in candidates if self.nodes[l].parents])))
            depth += 1
    ...
    def relativePos2(self, currentIdx, otherIdx):
        ...
        if True:
            ...
            if len(cpath) > 2 and len(opath) > 2:
                if cpath[-3] == opath[-3] and cpath[-2] != opath[
                    -2]:  # same grand parent not same parent
                    if not type:
                        type = 'SGP'
                    else:
                        raise Exception("Not mutual exclusive child type")
                if cpath[-3] == opath[-2] and opath[-1] != cpath[
                    -2]:  # current node's parent's brother
                    # return 'PB'
                    if not type:
                        type = 'PB'
                    else:
                        raise Exception("Not mutual exclusive child type")
                if opath[-3] == cpath[-2] and cpath[-1] != opath[
                    -2]:  # other node's parent's brother
                    # return 'rPB'
                    if not type:
                        type = 'rPB'
                    else:
    ...
    def get_possible_reentrance_constrained(self, currentIdx, currentChildIdx):
        ...
        if len(cur_p.children) > 1:
            result_set = set([sb for sb in cur_p.children if
                              sb != currentChildIdx and sb not in cur.parents and sb not in cur.children])
        '''
        cur_p = self.nodes[currentIdx]
        j=0
        ...
        result_set = result_set.union(set(gov for rel, gov in cur.incoming_traces if
                                          gov != currentChildIdx and gov in self.nodes and gov not in cur.parents and gov not in cur.children))
        # adding predicate
        if isinstance(currentChildIdx, int) and 'pred' in self.sent[currentChildIdx]:
            result_set = result_set.union(set(
                prd for prd in self.sent[currentChildIdx]['pred'] if
                prd != currentChildIdx and prd in self.nodes and prd not in cur.parents and prd not in cur.children))
        return result_set
    ...
    def get_possible_parent_constrained(self, currentIdx, currentChildIdx, i=2):
        ...
        if c > 0:
            ...
            while left_sp.children:
                ...
                if ls_r_child != currentChildIdx and ls_r_child not in self.nodes[
                    currentChildIdx].children and ls_r_child not in self.nodes[
                    currentChildIdx].parents:
        ...
        if c < len(children) - 1:
            ...
            while right_sp.children:
                ...
                if rs_l_child != currentChildIdx and rs_l_child not in self.nodes[
                    currentChildIdx].children and rs_l_child not in self.nodes[
                    currentChildIdx].parents:
        ...
        while cur_p.parents and j < i:
            cur_gp = cur_p.parents[0]
            if cur_gp != currentChildIdx and cur_gp not in self.nodes[
                currentChildIdx].children and cur_gp not in self.nodes[
                currentChildIdx].parents:
        ...
        if isinstance(currentChildIdx, int) and 'pred' in self.sent[currentChildIdx]:
            candidate_parents = candidate_parents.union(set(
                prd for prd in self.sent[currentChildIdx]['pred'] if
                prd != currentChildIdx and prd in self.nodes and prd not in cur.parents and prd not in cur.children))
        return candidate_parents
    ...
    def get_possible_children(self, currentIdx):
        ...
        for otherIdx in self.nodes:
            if otherIdx != 0 and otherIdx != currentIdx and \
                ...
                if len(cpath) > 2 and len(opath) > 2 and cpath[-3] == opath[-3] and cpath[
                    -2] != opath[-2]:
                    possible_children.append('SGP' + str(num_SGP))
                    num_SGP += 1

                if len(cpath) > 2 and len(opath) > 2 and cpath[-3] == opath[-2] and opath[
                    -1] != cpath[-2]:
                    possible_children.append('PB' + str(num_PB))
                    num_PB += 1

                if len(cpath) > 2 and len(opath) > 2 and cpath[-2] == opath[-3] and cpath[
                    -1] != opath[-2]:
    ...
    def flipConst(self):
        ...
        while parsed_tuples:
            ...
            #    import pdb
            #    pdb.set_trace()
            if (isinstance(self.nodes[parent].tag, ConstTag) or r'/' in self.nodes[
                parent].tag) and not isinstance(self.nodes[child].tag, ConstTag):
            ...
            elif isinstance(self.nodes[parent].tag, ConstTag) and isinstance(
                    self.nodes[child].tag, ConstTag):
                for p in self.nodes[parent].parents[:]:
                    if p != child:
                        self.add_edge(p, child)
    ...
    def print_tuples(self, bfs=False):
        ...
        if not self.sent:
            if bfs:
                return '\n'.join("(%s(%s-%s),(%s-%s))" % (
                self.get_edge_label(g, d), ','.join(w for w in self.nodes[g].words), g,
                ','.join(t for t in self.nodes[d].words), d) for g, d in self.bfs())
            else:
                return '\n'.join("(%s(%s-%s),(%s-%s))" % (
                self.get_edge_label(g, d), ','.join(w for w in self.nodes[g].words), g,
                ','.join(t for t in self.nodes[d].words), d) for g, d in self.tuples())
        else:
            output = ''
            if bfs:
                seq = self.bfs()
            ...
            for g, d in seq:
                g_span = ','.join(
                    tok['form'] for tok in self.sent[g:self.nodes[g].end]) if isinstance(
                    g, int) else ','.join(self.nodes[g].words)
                d_span = ','.join(
                    tok['form'] for tok in self.sent[d:self.nodes[d].end]) if isinstance(
                    d, int) else ','.join(self.nodes[d].words)
                output += "(%s(%s-%s:%s),(%s-%s:%s))\n" % (
                self.get_edge_label(g, d), g_span, g, self.nodes[g].tag, d_span, d,
                self.nodes[d].tag)
    ...
    def print_dep_style_graph(self):
        ...
        for g, d in self.tuples():
            span_g = ','.join(
                tok['form'] for tok in self.sent[g:self.nodes[g].end]) if isinstance(g,
                                                                                     int) else ','.join(
                self.nodes[g].words)
            span_d = ','.join(
                tok['form'] for tok in self.sent[d:self.nodes[d].end]) if isinstance(d,
                                                                                     int) else ','.join(
                self.nodes[d].words)
            tag_g = '%s%s' % (self.get_node_tag(g), self.nodes_error_table[g])
            tag_d = '%s%s' % (self.get_node_tag(d), self.nodes_error_table[d])
            edge_label = '%s%s' % (
            self.get_edge_label(g, d), self.edges_error_table[(g, d)])
            tuple_str += "%s(%s-%s:%s, %s-%s:%s)\n" % (
            edge_label, span_g, index_list.index(g), tag_g, span_d, index_list.index(d),
            tag_d)
